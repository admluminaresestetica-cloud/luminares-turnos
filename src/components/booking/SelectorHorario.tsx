...
export default function SelectorHorario({ slots, horaSeleccionada, onSelect, cargando }: Props) {
  if (cargando) {
    return (
      <div className="bg-slate-50/50 border border-slate-200/80 rounded-2xl p-6 sm:p-8 flex flex-col items-center justify-center gap-2">
  ...
  if (slots.length === 0) {
    return (
      <div className="bg-amber-50/60 border border-amber-200/80 rounded-2xl p-3.5 sm:p-4 flex items-start gap-2.5 sm:gap-3">
      ...
      {slots.map((hora) => {
        ...
        return (
          <button
            ...
            className={`
              py-2.5 sm:py-3 px-2 sm:px-3 rounded-xl text-xs sm:text-sm font-bold border transition-all flex items-center justify-center gap-1 sm:gap-1.5 active:scale-95
              ${activa
