...
import { ArrowLeft, Calendar, ChevronRight, CheckCircle2, Loader2 } from 'lucide-react';

import FormConfirmacion from '@/components/booking/FormConfirmacion';
import ModalReservaExitosa, { DatosReservaExitosa } from '@/components/booking/ModalReservaExitosa';
import PasoSeleccionFechaHora from '@/components/booking/PasoSeleccionFechaHora';
...
export default function FlujoAgendaConfirmacion({
  ...
  const { descuentoMonto, referidoValido, mensajeReferido } = useReferidos(codigoReferidoUsado, configSistema, precioTotal);
  ...
  const cargarSlots = useCallback(async (fechaSel: string) => {
    ...
    const disponibles = calcularSlotsDisponibles(fechaSel, duracionTotal, configCalendario.horarios_atencion, reservas);
  ...
  const prepararReservaBase = async () => {
    ...
    return { reserva, nombreLimpio, precioFinal, codigoReferidoPropio, codigoUsadoLimpio };
  };
      ...
      <main className="min-h-screen bg-[#edf0ec] dark:bg-zinc-950 flex items-center justify-center p-4 sm:p-6">
        <div className="bg-white/90 dark:bg-zinc-900 border border-stone-200/80 dark:border-zinc-800 rounded-2xl p-5 sm:p-6 shadow-xs max-w-sm w-full text-center">
          ...
          {volverHref ? (
            ...
            <button type="button" onClick={onVolver} className={`text-xs font-bold ${styles.link}`}>Volver al inicio</button>
          ) : null}
        </div>
      </main>
    );
  ...
  const diasSemana = (() => {
    ...
    return activos.length > 0 ? activos : [0, 1, 2, 3, 4, 5, 6];
  })();
        ...
        <div className="bg-white/90 dark:bg-zinc-900 backdrop-blur-sm border border-stone-200/80 dark:border-zinc-800 rounded-[28px] p-4 sm:p-7 shadow-xs">
          ...
          {paso === 'agenda' && (
            <PasoSeleccionFechaHora
              ...
              summaryBgStyle={styles.summaryBg}
