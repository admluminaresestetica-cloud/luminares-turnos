...
export default function BuscadorMulticoincidencia({
  ...
  return (
    <div className="w-full rounded-2xl border border-slate-200/85 bg-white p-4 shadow-sm transition-all dark:border-zinc-800 dark:bg-zinc-900 sm:p-5">
      <form onSubmit={handleBuscar} className="flex flex-col gap-2 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          <Search className="pointer-events-none absolute left-3.5 top-1/2 h-4.5 w-4.5 -translate-y-1/2 text-slate-400 dark:text-zinc-500" />
          <input
            type="text"
            placeholder="Buscar por nombre, teléfono o código..."
            value={termino}
            onChange={(e) => setTermino(e.target.value)}
            className="h-12 w-full rounded-xl border border-slate-200 bg-slate-50/60 pl-10 pr-3 text-sm text-slate-800 placeholder:text-slate-400 outline-none transition-all focus:border-teal-500 focus:bg-white focus:ring-2 focus:ring-teal-500/20 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-100 dark:placeholder:text-zinc-500 dark:focus:border-teal-400 dark:focus:ring-teal-400/20"
          />
        </div>
        <button
          type="submit"
          disabled={cargando}
          className="flex h-12 shrink-0 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 px-6 text-sm font-bold text-white shadow-lg shadow-emerald-600/20 transition-all hover:from-emerald-500 hover:to-teal-500 active:scale-[0.98] disabled:opacity-50 disabled:shadow-none"
        >
      ...
      {resultados.length > 0 && (
        <div className="mt-4 space-y-1 border-t border-slate-100 pt-3 dark:border-zinc-800">
          <div className="mb-1 flex items-center gap-1.5 px-2 text-[11px] font-semibold uppercase tracking-wider text-slate-400 dark:text-zinc-500">
            <Users className="h-3.5 w-3.5" />
            {resultados.length} resultado{resultados.length > 1 ? 's' : ''}
          </div>

          <div className="max-h-80 divide-y divide-slate-100 overflow-y-auto dark:divide-zinc-800/80 sm:max-h-96">
            {resultados.map((item, idx) => (
              <div
                key={item.pacienteFicha?.id || item.reservaHoy?.id || idx}
                className="flex flex-col gap-3 rounded-xl px-2 py-3 transition-colors hover:bg-slate-50/80 dark:hover:bg-zinc-800/50 sm:flex-row sm:items-center sm:justify-between"
              >
                <div className="flex min-w-0 flex-col gap-1.5">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="text-sm font-semibold text-slate-800 dark:text-zinc-200">{item.nombre}</span>
                    <span className="rounded-full border border-slate-200/80 bg-slate-50 px-2 py-0.5 text-[11px] font-medium text-slate-600 dark:border-zinc-800 dark:bg-zinc-800 dark:text-zinc-400">
                      {item.detalleZona}
                    </span>
                  </div>
                ...
                <div className="flex shrink-0 items-center gap-2">
                  {item.pacienteFicha?.id && (
                    <button
                      ...
                      className="inline-flex h-10 items-center gap-1.5 rounded-lg border border-slate-200/80 bg-slate-50 px-3 text-xs font-semibold text-slate-700 transition-all hover:bg-slate-100 hover:text-slate-900 active:scale-95 dark:border-zinc-800 dark:bg-zinc-800/60 dark:text-zinc-300 dark:hover:bg-zinc-800 dark:hover:text-zinc-100"
                    >
