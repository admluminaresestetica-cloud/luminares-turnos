...
export default function ModalHistorialSesiones({
  ...
  const cargarHistorial = async () => {
    setLoading(true);
    try {
      // Corregido: consultando a la tabla correcta
          ...
          <div className="space-y-4">
            <div className="flex flex-wrap gap-2 border-b pb-3">
              {sesiones.map((sesion, index) => {
                ...
                const fecha = sesion.updated_at ? new Date(sesion.updated_at).toLocaleDateString() : 'Reciente';
                ...
                return (
                  <button
                    ...
                    className={`text-xs px-3 py.5 rounded-lg border font-semibold transition-all ${
                      esActiva
                        ? 'bg-indigo-600 text-white border-indigo-600'
                        : 'bg-slate-50 text-slate-700 border-slate-200 hover:bg-slate-100'
                    }`}
                  >
                    Sesión Registrada ({fecha})
                  </button>
                );
              })}
            </div>
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 space-y-3 text-xs">
                <div className="flex justify-between items-center border-b pb-2">
                  <span className="font-bold text-slate-800">
                    Actualizado: {sesionSeleccionada.updated_at ? new Date(sesionSeleccionada.updated_at).toLocaleString() : 'N/D'}
                  </span>
                </div>
