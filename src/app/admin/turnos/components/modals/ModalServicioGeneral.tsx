...
interface ModalServicioGeneralProps {
  ...
  onClose: () => void
  onSaveSuccess: () => void // Callback para recargar la tabla en el padre y cerrar
}
...
export default function ModalServicioGeneral({
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      setErrorMensaje('Ocurrió un error al subir la imagen. Revisa los permisos del Bucket.')
    } finally {
      setUploading(false)
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        precio: Number(servicioGeneralEdit.precio ?? 0),
        duracion_minutos: Number(servicioGeneralEdit.duracion_minutos ?? 0),
        ...
        icono: servicioGeneralEdit.icono
      }
      ...
      setErrorMensaje(error.message || 'Ocurrió un error al guardar el servicio.')
    } finally {
      setGuardando(false)
    }
  }
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-400 mt-1 leading-relaxed dark:text-zinc-500">
              Podés usar cualquier ícono de Lucide. Buscá más en{' '}
              <a 
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              ...
              <label className={`cursor-pointer flex-1 ${uploading || guardando ? 'pointer-events-none opacity-50' : ''}`}>
          ...
          <div className="p-3 bg-gray-50 rounded-2xl border border-gray-100 dark:bg-zinc-800/50 dark:border-zinc-800">
            <label htmlFor="servicioGeneralActivo" className="flex items-center gap-2 text-xs font-medium text-gray-700 cursor-pointer dark:text-zinc-300 select-none">
              <input
                type="checkbox"
          ...
          <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-2 sm:gap-2.5 pt-3 border-t border-gray-100 dark:border-zinc-800 sticky bottom-0 bg-white dark:bg-zinc-900 sm:static sm:bg-transparent -mx-5 sm:mx-0 px-5 sm:px-0 pb-1 sm:pb-0">
            ...
            <button
              type="submit"
              disabled={uploading || guardando}
              className="w-full sm:w-auto px-5 py-3 sm:py-2.5 text-xs font-semibold bg-gray-900 hover:bg-gray-800 disabled:opacity-50 text-white rounded-xl transition-all shadow-sm active:scale-95 dark:bg-zinc-100 dark:text-zinc-900 dark:hover:bg-zinc-200 flex items-center justify-center gap-2"
            >
